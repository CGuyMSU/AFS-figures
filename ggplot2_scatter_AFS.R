...
#create some data for making the figure purposes
...
#default ggplot figure with an annotated label
len_wt_default <- ggplot(length_weight_data, aes(x = length, y = weight, fill = species)) + 
  geom_point() +
  labs(title = "A")
len_wt_default

#ggplot figure that meets AFS style guidelines
len_wt_afs <- ggplot(length_weight_data, aes(x = length, y = weight, fill = species)) +
  #symbol shape and size you can find number definition using a google search
  ...
  xlab("Length (mm)") +  
  #add B to figure and legend title left blank here because we want a legend but no title
  labs(title = "B", fill = "") + 
  ...
  theme (axis.title.y = element_text(size = 14, margin = margin(t = 0, r = 5, b = 0, l = 0), colour = "black"), 
         axis.title.x = element_text(size = 14, margin = margin(t = 5, r = 0, b = 0, l = 0), colour = "black"), 
         #sets the font type
         text = element_text(family = "Times New Roman"), 
         #positions the legend on the figure
         ...
         #the axis size, color, and end shape
...
ggsave(file = "ggplot_figure.jpeg", new, width = 30.48, height = 22.86, units = "cm", dpi=200) 
